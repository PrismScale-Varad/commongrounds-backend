...
async def google_login_url(request: Request):
    authorization_url = create_authorization_url("google")
    return JSONResponse({"authorization_url": authorization_url})


@router.get("/apple/login")
async def apple_login_url(request: Request):
    authorization_url = create_authorization_url("apple")
    return JSONResponse({"authorization_url": authorization_url})


@router.get("/github/login")
async def github_login_url(request: Request):
    authorization_url = create_authorization_url("github")
    return JSONResponse({"authorization_url": authorization_url})


async def handle_oauth_callback(
    request: Request, db: Session, provider_name: str
) -> JSONResponse:
    try:
...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    return await handle_oauth_callback(request, db, "google")


@router.get("/apple/callback")
async def apple_callback(request: Request, db: Session = Depends(get_db)):
    return await handle_oauth_callback(request, db, "apple")


@router.get("/github/callback")
async def github_callback(request: Request, db: Session = Depends(get_db)):
    return await handle_oauth_callback(request, db, "github")
